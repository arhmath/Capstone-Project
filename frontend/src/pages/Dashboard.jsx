    ...
    <div className="flex min-h-screen bg-[#FDFCFB]">
      ...
      <aside className="w-72 bg-white border-r border-mq-peach/40 flex flex-col sticky top-0 h-screen z-20">
        ...
        <div className="mx-4 mb-4 p-4 bg-mq-primary/5 rounded-2xl border border-mq-primary/10">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="font-black text-slate-800 text-sm sm:text-base leading-tight truncate">
                {user?.username || 'Petualang'}
              </p>
              <p className="text-xs sm:text-[12px] font-bold text-mq-primary uppercase tracking-wider">
                Lv.{user?.level || 1} · {user?.jenjang || 'SD'}
              </p>
            </div>
          </div>
          <div className="mt-3">
            <div className="flex justify-between text-xs sm:text-[12px] font-bold text-slate-400 mb-1">
        ...
        <nav className="flex-1 px-4 mt-2">
          <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4 ml-4">Menu Utama</p>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  isActive 
                    ? 'bg-mq-primary text-white shadow-xl shadow-blue-200' 
                    : 'text-slate-600 hover:bg-mq-peach/30 hover:text-mq-primary'
                  }`}
                >
                  {link.icon}
